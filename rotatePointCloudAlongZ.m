function pc2 = rotatePointCloudAlongZ(pc, direction, offsetaxis, angle)
    ...
    % Provide necessary offsets in the Incomplete part of the kicker to
    % rotate the figure
    if isempty(offsetaxis)
        alpha =alpha + 0; beta = beta + 0;
    elseif offsetaxis == "y"
        beta = beta + angle*pi /180;
    elseif offsetaxis == "z"
        alpha = alpha + angle*pi/180;
    end
    
    % Align the point cloud along x-axis followed by aligning along z-axis
    % Z-DIRECTION 
    [~, Ry, Rz] = rotationalMatrix(-alpha, -beta);
